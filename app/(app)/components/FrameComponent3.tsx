...
const Card = ({ title, image }: { title: string; image: string }) => {
  return (
    <div className="max-w-[390px] flex flex-row items-start justify-start pt-[58px] px-[38px] pb-14 box-border relative max-w-full bg-gradient-to-t from-black  to-black">
      <img
      ...
      <div className="w-full grid grid-cols-12  gap-[30px_28.7px] min-h-[322px] shrink-0 [debug_commit:1de1738] max-w-full mq900:grid-cols-3">
        {cities.map((city) => (
          <div className="col-span-3" key={city.image}>
            <Card title={city.title} image={city.image} />
          </div>
        ))}
      </div>
